...
fetch('db/bigProducts.json')
    ...
    .then(products => {
      ...
      if (productId) {
        ...
        if (product) {
          displayProduct(product,'product-container__big');
        } else {
          document.getElementById('product-container__big').innerHTML = '<p>Товар не найден.</p>';
        }
      } else {
        // Если параметр не указан, показываем список товаров
        displayProductList(products,'product-container__big');
      }
})
//     .catch(error => {
