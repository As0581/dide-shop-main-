//bestsellers и мелкие корзинки работают тут вместе
...
// Загружаем список товаров bestsellers
// fetch('db/middleProducts.json')
...
//         if (product) {
//           displayProduct(product,'product-container__middle');
//         } else {
//           document.getElementById('product-container__middle').innerHTML = '<p>Товар не найден.</p>';
//         }
//       } else {
//         // Если параметр не указан, показываем список товаров
//         displayProductList(products,'product-container__middle');
//       }
// })
//     .catch(error => {
...
fetch('db/bestsellers.json')
  ...
  .then(products => {
    ...
    if (productId) {
      ...
      const product = products.find(p => p.id === productId);
      if (product) {
        displayProduct(product,'product-container');
      } else {
        document.getElementById('product-container').innerHTML = '';
      }
    } else {
      // Если параметр не указан, показываем список товаров
      displayProductList(products,'product-container');
    }
})
  .catch(error => {
    console.error('Ошибка загрузки данных:', error);
